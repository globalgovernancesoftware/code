...
//$partialTryNumber=0;
function downloadGeoData($fol,$connection,$statusType,$tryNumber)
{
	...
	if(isset($mainId))
	{
		if($statusType==2)
		{
			...
			else
			{
				...
				for($i=0;$i<count($address);$i++)
				{
					...
					if($countIt>1)
					{
						$finAddress=$newAddressA[$countIt-3]."+".$newAddressA[$countIt-2]."+".$newAddressA[$countIt-1];
					}
		...
		for($i=0;$i<count($finalId);$i++)
		{
			...
			$apiQueryArray[]="https://maps.googleapis.com/maps/api/geocode/json?address=".$finalAddress[$i]."&key=".$api;
			//echo $apiQuery."\n";
		...
		$div=10;
		$countDone=0;
		$chunked_urls = array_chunk($apiQueryArray,$div); 		
		$z=0;
		print_R($apiQueryArray);
...
$resultDownloaded2="Yes";

$tNumber=0;
$resultDownloaded=downloadGeoData($folder,$con,0,$tNumber);
...
$resultDownloaded2=downloadGeoData($folder,$con,2,$tNumber);
