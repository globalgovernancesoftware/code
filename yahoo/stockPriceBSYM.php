...
$query="select ID_BB_SEC_NUM_DES,FEED_SOURCE,ID_BB_GLOBAL from (SELECT * FROM bsym.master where FEED_SOURCE in ('CN','US') and SECURITY_TYP in ('Common Stock','REIT','Equity WRT') ) as t  where (length(ID_BB_SEC_NUM_DES)<7 or ID_BB_SEC_NUM_DES  like '%WS%') order by ID_BB_SEC_NUM_DES;";
...
for($i=0;$i<$countTicker;$i++)
{
	...
	if($countData>1)
	{
		...
		$query="insert into nsMine.yahooSp values ".substr($finalData,0,-1);
		$queryIt=mysqli_query($con,$query) ;
