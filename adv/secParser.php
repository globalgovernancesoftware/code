...
function loadXML($url)
{
	...
	$sel=mysqli_Select_db($con, "novaGlobalADV");	
		...
		foreach($indList->Firm as $Firm) 
		{
			...
			$SECRgnCD[]=mysqli_real_escape_String($con,@(string) $Info['SECRgnCD']);
			...
			$MailingAddr_Strt2[]=mysqli_real_escape_String($con,@(string) $MailingAddr['Strt2']);
			  $Rgstn=$Firm->Rgstn->attributes();
			$Rgstn_FirmType[]=mysqli_real_escape_String($con,@(string) $Rgstn['FirmType']);
			$Rgstn_St[]=mysqli_real_escape_String($con,@(string) $Rgstn['St']);
			$Rgstn_Dt[]=mysqli_real_escape_String($con,@(string) $Rgstn['Dt']);
			...
			$Q1I[]=mysqli_real_escape_String($con,@(string) $FormInfo['Q1I']);
			$Q1M[]=mysqli_real_escape_String($con,@(string) $FormInfo['Q1M']);
			$Q1N[]=mysqli_real_escape_String($con,@(string) $FormInfo['Q1N']);
			$Q1O[]=mysqli_real_escape_String($con,@(string) $FormInfo['Q1O']);
			$Q1P[]=mysqli_real_escape_String($con,@(string) $FormInfo['Q1P']);
			$CIKNb[]=mysqli_real_escape_String($con,@(string) $FormInfo['CIKNb']);
			  $Item2A=$Firm->FormInfo->Part1A->Item2A->attributes();
			$Q2A1[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A1']);
			$Q2A2[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A2']);
			$Q2A3[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A3']);
			$Q2A4[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A4']);
			$Q2A5[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A5']);
			$Q2A6[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A6']);
			$Q2A7[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A7']);
			$Q2A8[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A8']);
			$Q2A9[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A9']);
			$Q2A10[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A10']);
			$Q2A11[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A11']);
			$Q2A12[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A12']);
			$Q2A13[]=mysqli_real_escape_String($con,@(string) $Item2A['Q2A13']);
			  $Item2B=$Firm->FormInfo->Part1A->Item2B->attributes();
			$Q2B1[]=mysqli_real_escape_String($con,@(string) $Item2B['Q2B1']);
			$Q2B2[]=mysqli_real_escape_String($con,@(string) $Item2B['Q2B2']);
			$Q2B3[]=mysqli_real_escape_String($con,@(string) $Item2B['Q2B3']);
			  $Itemrg=$Firm->FormInfo->Part1A->Itemrg->attributes();
			$OrgFormNm[]=mysqli_real_escape_String($con,@(string) $Itemrg['OrgFormNm']);
			$OrgFormOthNm[]=mysqli_real_escape_String($con,@(string) $Itemrg['OrgFormOthNm']);
			  $Item3B=$Firm->FormInfo->Part1A->Item3B->attributes();
			$Q3B[]=mysqli_real_escape_String($con,@(string) $Item3B['Q3B']);
			  $Itemta=$Firm->FormInfo->Part1A->Itemta->attributes();
			$StateCD[]=mysqli_real_escape_String($con,@(string) $Itemta['StateCD']);
			  $Itemnt=$Firm->FormInfo->Part1A->Itemnt->attributes();
			$CntryNm[]=mysqli_real_escape_String($con,@(string) $Itemnt['CntryNm']);
			  $Itemtl=$Firm->FormInfo->Part1A->Itemtl->attributes();
			$TtlEmp[]=mysqli_real_escape_String($con,@(string) $Itemtl['TtlEmp']);
		...
		//StateRgstn
			if($Firm->NoticeFiled->States)
			{
				foreach($Firm->NoticeFiled->States as $NoticeFiled)
				{
					$Stat=$NoticeFiled->attributes();
					$NoticeFiled_RgltrCd[$i][]=mysqli_real_escape_String($con,@(string) $Stat['RgltrCd']);
					$NoticeFiled_St[$i][]=mysqli_real_escape_String($con,@(string) $Stat['St']);
					$NoticeFiled_Dt[$i][]=mysqli_real_escape_String($con,@(string) $Stat['Dt']);
				}				
			}
	...
	for($i=0;$i<count($FirmCrdNb);$i++)
	{
		...
		echo "Info".$i."\n";
			...
			$query.="('".$SECRgnCD[$i]."','".
						...
						$Q11H1C[$i]."','".
						$Q11H2[$i]."'),";
			$queryIt="INSERT INTO secADV values ".substr($query,0,-1).";";
	...
	{	
		...
		{	
			...
			{		
				...
				$queryIt="INSERT INTO secADVWeb values ".substr($query,0,-1).";";
	...
	for($i=0;$i<count($FirmCrdNb);$i++)
	{
		for($j=0;$j<count(@$Rgstn_FirmType[$i]);$j++)
		{
			if($i%5000==0)
			{			
				echo "secADVNoticeFiled".$i."\n";
				$query.="('".$FirmCrdNb[$i]."','".$Rgstn_FirmType[$i][$j]."','".$Rgstn_St[$i][$j]."','".$Rgstn_Dt[$i][$j]."'),";		
				$queryIt="INSERT INTO secADVNoticeFiled values ".substr($query,0,-1).";";
...
loadXML('IA_FIRM_SEC_Feed_02_23_2016.xml');
?>
